...
function check_subscribe_form(frm) {
    if (emailCheck(trim(frm.email.value)) != '1') {
        frm.email.style.background = 'yellow';
        return false;
    } else {
        frm.email.style.background = '#FFF';
    }
    
    // submit the subscribe form
    $.post(_HTTP_ROOT + '/source/subscribe/', $(frm).serialize(), function(res) {
        $('.source_email_window').fadeOut();
        document.location.href=_HTTP_ROOT + '/source/download/' + $(frm).find('input[name=sec_key]').val();
    })
...
function open_subscribe_window() {
    $('.source_email_window').fadeIn().find('input[name=email]').focus();
}
