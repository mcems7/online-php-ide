...
var opened_files_sequence = {
	...
	__fix_file_param: fix_file_param, // pointer to a function
...
function file_is_in_right_panel(file) {
	var retVal = false;
    file = fix_file_param(file);
	$('.right_panel li a').each(function(i, obj) {
		if ($(obj).attr('title') == file) {
			retVal = true;
			return false;
		}
...
function add_file_to_right_panel(file) {
    ...
    file = fix_file_param(file);
	var pure_file_name = file.match(/([^\/]+)$/g, '/');

	$('.right_panel ul').append(
        $('<li />').append(
            $('<a href="javascript:;" title="'+file+'"></a>')
            .html(pure_file_name+'<br><span>'+file+'</span>')
            .unbind('click').click(function(){
                switch_to_file($(this).attr('title'));
                $(this).addClass('selected');
            })
		)
        .css('position', 'relative')
		.append($('<span>x</span>').attr("class", "close_file_x"))
		);
	...
	.click(function() {
		close_file($(this).parent().find('a').attr('title'));
	});
}
...
function switch_to_file(file) {
	...
	// check which file is selected now AND if it's not the required one, and save the code
	if ($('.right_panel a.selected').length && $('.right_panel a.selected').attr('title') != file) {
		var arr_index = $('.right_panel a.selected').attr('title');
	...
	var rExtension = file.substr(file.lastIndexOf('.') + 1);
	
	if (typeof(files_code[file]) != 'undefined' && files_code[file] != null) {
		editor.setValue(files_code[file]["content"]);
		editor.setCursor(files_code[file]["cursor"]);
	} else {
	...
	opened_files_sequence.add(file);

	$('.bottom_toolbar .filename').text(file);
	editor.focus();
...
function close_file(file) {
	...
	if (!file) {
		if ($('.right_panel li a.selected').length > 0) {
			file = $('.right_panel li a.selected').attr('title');
		}
	}
	...
	$('.right_panel a').each(function(i,obj) {
		if ($(obj).attr('title') == file) {
			...
			opened_files_sequence.remove(file);
...
function save_file() {
	...
	// get the filename
	var fName = $('.right_panel a.selected').attr('title');
	// get the content
...
function rename_directory() {
    ...
    $.post(_HTTP_ROOT + '/ajax/rename_directory.php', {old_name:old_name, new_name:new_name}, function(res) {
        ...
        if ($('.left_panel #'+old_name.replace(/\//g, '___')).parents('li').eq(0).length) {
            $('.left_panel #'+old_name.replace(/\//g, '___')).parents('li').eq(0).find('li').remove();
        }
    });
    close_modal_window();
}
...
function adjust_ide_size() {
    ...
    $('.resize_arrow_left').css('right', '191px');
    
    // resize the main panel with the whole window
	$('.main_inner').css('width', new_width + 'px');
	$('.main_panel, .toolbar_main_panel').css('width', (new_width - 400) + 'px');
...
$(function(){
	$(".left_panel").jstree({
		...
		"json_data" : {
			"ajax" : {
				...
				"data" : function (n) {
					window.last_opening_location = n.attr ? n.attr("id").replace(/\/\//g, '/').replace(/___/g,"/") : '';
					return {
						"operation" : "get_children",
						"id" : n.attr ? fix_file_param(n.attr("id").replace("node_","")) : 0
					};
				}
			}
		}
	...
	.bind('mousedown', function(e) {
		...
		$('.ide').unbind('mousemove').bind('mousemove', function(e) {
        	...
        	$('.ide').unbind('mousemove').unbind('mouseup');
    		window.resize_action = false;
        });
	});
